...
if __name__ == '__main__':
    with app.app_context():
      ...
      #   db.session.commit()
      #   print("Đã thêm dữ liệu mẫu vào các bảng.")
      #   import hashlib
      #
      #   trang_thai_phong = [
      #       TrangThaiPhong( tenTrangThai="Trống"),
      #       TrangThaiPhong(tenTrangThai="Đang sử dụng"),
      #       TrangThaiPhong(tenTrangThai="Đang bảo trì"),
      #   ]
      # #
      # #   loai_khach_hang = [
      ...
      # #       NhanVien(maNhanVien=3, hoTen="Lê Văn C", cmnd="123456789012", diaChi="Đà Nẵng"),
      # #       NhanVien(maNhanVien=4, hoTen="Phạm Thị D", cmnd="321098765432", diaChi="Hải Phòng"),
      # #       NhanVien(maNhanVien=5, hoTen="Hoàng Văn E", cmnd="456789123456", diaChi="Cần Thơ"),
      # #       NhanVien(maNhanVien=6, hoTen="Nguyễn Thị F", cmnd="654321098765", diaChi="Huế"),
      # #       NhanVien(maNhanVien=7, hoTen="Đỗ Văn G", cmnd="567890123456", diaChi="Nha Trang"),
      # #       NhanVien(maNhanVien=8, hoTen="Vũ Thị H", cmnd="876543210987", diaChi="Vũng Tàu"),
      # #       NhanVien(maNhanVien=9, hoTen="Ngô Văn I", cmnd="789012345678", diaChi="Quảng Ninh"),
      # #       NhanVien(maNhanVien=10, hoTen="Trịnh Thị J", cmnd="901234567890", diaChi="Bình Dương"),
      # #   ]
      # #   import random
      # #   for i in range(1, 11):
      # #       db.session.add(KhachHang(
      # #           hoTen=f"Khách hàng {i}",
      # #           cmnd=f"{random.randint(100000000000, 999999999999)}",
      # #           diaChi=f"Địa chỉ {i}",
      # #           maLoaiKhach=random.randint(1, 10)
      # #       ))
      # # #
      # #   for i in range(1, 11):
      # #       db.session.add(Phong(
      # #           trangThaiPhong=random.randint(1, 10),
      # #           maLoaiPhong=1 if i%2==0 else 2
      # #       ))
      # # #
      # #   phieu_dat_phong = [
